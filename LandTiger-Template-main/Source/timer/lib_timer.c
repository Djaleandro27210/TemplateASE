...
0	Interrupt	Viene generata l'interruzione, ma il timer continua a contare senza fermarsi né resettarsi. (Utile per campionamenti continui).
1	Interrupt + Stop	Viene generata l'interruzione e il timer si mette in PAUSA. Devi riavviarlo tu via software.
2	Interrupt + Reset	Viene generata l'interruzione e il timer torna a 0, ma riparte immediatamente a contare.
3	Interrupt + Reset + Stop	Viene generata l'interruzione, il timer torna a 0 e si ferma. È la modalità "One-shot" completa.
...
/******************************************************************************
** Function name:    setup+blinka_led
**
** Parameters:       numtimer, mr1,mr2,frequenza del blink, frequenza del timer complessiva(25000000)
**
** Returned value:   None
**
** Notes:            controlla la frequenza timer da startup.
******************************************************************************/
void Setup_TIMER_BLINK(int timer_num, int mr_off, int mr_reset,int frequenza_desiderata,int frequenza_timer) {
    
    // 1. Configura MR_OFF: genera solo interrupt, non resetta il timer
    // Il parametro '1' indica l'interrupt (bit 0 del MR Control Register)
	int time_total=frequenza_timer / frequenza_desiderata;

	int time_on=time_total/2;
    init_timer(timer_num, 0, mr_off, 1, time_on);
    
    // 2. Configura MR_RESET: genera interrupt E resetta il timer (bit 0 + bit 1 = valore 3)
    // Il parametro '3' indica Interrupt + Reset (TC si azzera e ricomincia)
    init_timer(timer_num, 0, mr_reset, 3, time_total);
    
    // 3. Abilita il timer
    enable_timer(timer_num);
}
///da usare con questo (inserire nel timerhandler)
//la chiamata tipica: 	PWM_Handle_Interrupt(LPC_TIM3, 0, 1, 2);
//se vuoi far blinkare tutti i led, led_num=150

void Getione_Blink(LPC_TIM_TypeDef *TIMx, uint8_t mr_off, uint8_t mr_on, uint8_t led_num) {
    
    // Controllo se l'interrupt è stato causato dal Match Register di SPEGNIMENTO
    if (TIMx->IR & (1 << mr_off)) {
			if(led_num==150){
				LED_OffAll();
			}
			else {
        LED_Off(led_num);      				// Spegni il LED
			}
        TIMx->IR = (1 << mr_off);   // Pulisci il flag di interrupt scrivendo 1
    }
    
    // Controllo se l'interrupt è stato causato dal Match Register di ACCENSIONE/RESET
    else if (TIMx->IR & (1 << mr_on)) {
			if(led_num==150){
				LED_onAll();
			}
			else{
        LED_On(led_num);            // Riaccendi il LED
			}
        TIMx->IR = (1 << mr_on);    // Pulisci il flag di interrupt
    }
}
